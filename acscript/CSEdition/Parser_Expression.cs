...
namespace ACScript
{
    ...
    partial class Parser
    {
        ...
        IExpression parseAssignmentExpression()
        {
            ...
            return new AssignmentExpression(opKind, leftExpr, rightExpr);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public AssignmentExpression.OpKind parseAssignmentOperator()
        {
        ...
        FunctionCallExpression parseFunctionCallExpression()
        {
            ...
            return new FunctionCallExpression(sequenceExpr);
        }
        ...
        {
            // "delete"
            System.Diagnostics.Debug.Assert(currentToken().Value == Token.Kind.KeyDelete);
            nextToken();
        ...
        IExpression parsePostfixExpression()
        {
            ...
            while (firstExpr != null)
            {
                ...
                else if (currentToken().Value == Token.Kind.OpLBracket)
                {// IndexExpression
                    secondExpr = parseIndexExpression();
                }
                else if (currentToken().Value == Token.Kind.OpLParen)
                {// FunctionCallExpression
                    secondExpr = parseFunctionCallExpression();
                }
                else if (currentToken().Value == Token.Kind.OpDot)
                {// ChildIdentExpression
                    ...
                    secondExpr = new ChildIdentExpression(new Identifier(currentToken()));
                    nextToken();
                }
                else
                {
                ...
                {
                    firstExpr = new PostfixExpression(opKind, firstExpr);
                }
                else if (secondExpr != null)
                {
                    firstExpr = new PostfixExpression(firstExpr, secondExpr);
                }
                else
                {
                    return null;
        ...
        IExpression parseUnaryExpression()
        {
            // 単項演算子解析
            {
                ...
                if (opKind != UnaryOpExpression.OpKind.Unknown)
                {
                    ...
                    return new UnaryOpExpression(opKind, unaryExpr);
                }
            }
        ...
        UnaryOpExpression.OpKind parseUnaryOperator()
        {
            switch (currentToken().Value)
            {
                ...
                case Token.Kind.OpAt:
                    return UnaryOpExpression.OpKind.ObjectHandle;
        ...
        IExpression parseBitwiseAndExpression()
        {
            ...
            while (currentToken().Value == Token.Kind.OpAnd)
            {
                ...
                // 第1式を更新
                firstExpr = new BinaryOpExpression(BinaryOpExpression.OpKind.BitwiseAnd, firstExpr, secondExpr);
            }
            return firstExpr;
        }
        ...
        IExpression parseBitwiseXorExpression()
        {
            ...
            while (currentToken().Value == Token.Kind.OpXor)
            {
                ...
                // 第1式を更新
                firstExpr = new BinaryOpExpression(BinaryOpExpression.OpKind.BitwiseXor, firstExpr, secondExpr);
            }
            return firstExpr;
        }
        ...
        IExpression parseBitwiseOrExpression()
        {
            ...
            while (currentToken().Value == Token.Kind.OpOr)
            {
                ...
                // 第1式を更新
                firstExpr = new BinaryOpExpression(BinaryOpExpression.OpKind.BitwiseOr, firstExpr, secondExpr);
            }
            return firstExpr;
        }
        ...
        IExpression parseLogicalOrExpression()
        {
            ...
            while(currentToken().Value == Token.Kind.OpOrOr)
            {
                ...
                // 第1式を更新
                firstExpr = new BinaryOpExpression(BinaryOpExpression.OpKind.LogicalOr, firstExpr, secondExpr);
            }
            return firstExpr;
        }
