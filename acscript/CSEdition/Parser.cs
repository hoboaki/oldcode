...
namespace ACScript
{
    ...
    partial class Parser
    {
        ...
        public Parser( Lexer aLexer )
        {
            ...
            if (!parseResult)
            {
                System.Diagnostics.Debug.Assert(GetErrorKind() != ErrorKind.NONE);
            }

            // test
            if (parseResult)
            {
                moduleContext.Trace(new Tracer(), "moduleContext");
            }
        }
        ...
        //------------------------------------------------------------
        // BuildInType。
        static BuiltInType parseBuiltInType(Token aT)
        {
            switch (aT.Value)
            {
        ...
        bool parseStaticTypeStandardDef(StaticTypeDef aTD,bool aIsProtoType)
        {
            ...
            {// 継承
                ...
                while(true)
                {
                    ...
                    // デフォルト実装のパス
                    IdentPath implIdentPath = null;
                    if (currentToken().Value == Token.Kind.OpEqual)
                    {
                        implIdentPath = parseIdentPath();
                        if (implIdentPath == null)
                        {
                            setErrorKind(ErrorKind.STATIC_TYPE_DEF_IDENTIFIER_EXPECTED);
                            return false;
                        }
                    ...
                    // 登録
                    aTD.InheritInterfaceList.Add(new InheritInterface(typeIdentPath, implIdentPath));
        ...
        TypePath parseTypePath()
        {
            ...
            {// builtInType?
                ...
                {
                    typePath = new TypePath(builtinType);
                    nextToken();
                }
            }
            return typePath;
